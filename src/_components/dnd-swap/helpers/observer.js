...
let next;
let leftContainer = false;

/**
 * Tracks the dragged elements and performs the side effects when it is dragged over a drop zone (basically dispatching custom-events scrolling)
 * @param {Set<HTMLElement>} dropZones 
 * @param {HTMLElement} draggedEl 
 * @param {number} [INTERVAL_MS = INTERVAL_MS]
 */
export function observe(draggedEl, dropZones) {
    // initialization
    ...
    function andNow() {
        ...
        //     next = window.setTimeout(andNow, INTERVAL_MS);
        //     return;
        // }
        ...
        for (const dz of dropZones) {
            ...
            else if (index !== lastIndexFound || leftContainer) {
                leftContainer = false;
                indexObj.lastIndex = lastIndexFound;
